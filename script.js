...
(function() {
  ...
  if (secretArea) {
    secretArea.addEventListener('click', (e) => {
      // stop the click from triggering anything below it
      e.stopPropagation();
      openSecret();
    });
  }
  ...
  // close secret modal on Escape
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') closeSecret();
  });

  // also close if user clicks outside modal-content (if you want that UX)
  if (secretModal) {
